#!/usr/bin/env python3
"""S3 main capture — chosen-CT 'V' (sub-triggered vec_vec_mult_add) trace × N.

목적
====
S2 ('Z' = full indcpa_dec) 의 24400-sample window 가 sk shift loop + c1 shift loop
+ vec_vec_mult_add 가 섞여 단일 좌표 HW 모델이 noise-level (z=−1.5).

'V' 는 firmware 가 제공하는 sub-trigger wrapper :
  trigger_high() → vec_vec_mult_add(c2<<11, c1<<8, sk_unshifted, mod=8) → trigger_low()

V trace 의 24400 samples 는 *오직* vec_vec_mult_add 만 cover. setup loop 와
round_t 가 trigger window 밖.

논리 정합성 (잊지 말 것)
====
- 'V' 는 attack-valid 가 아니다. firmware 가 노출한 instrumented wrapper.
- 'V' 응답 µ′ (= 'Z' 와 byte-for-byte 일치, S3 round-trip 통과) 는 SCA 분석에
  사용 안 함. 단, capture sanity (chosen-CT 가 보드와 동일한지) 를 위한 round-trip
  검증에만 사용.
- 'V' 의 contribution 은 *trigger window purity 만*. 만약 sign≥88% 가 V 에서
  살아나면, 다음 단계는 sub-trigger 없이 'D'/'Z' 의 자연 trace 에서 같은 window
  를 외부 alignment 로 잘라 모델 재적용 — 그쪽이 진짜 attack-valid 검증.

S1 ('T') 와 동일한 capture 파라미터:
  - clkgen_x4, samples=24400, gain=25.0
  - 같은 보드, 같은 firmware (smaug1)
  - chosen-CT 의 c1 = 4·X^0 component 0 (S2 와 동일) → vec_vec_mult_add 가
    `4 * sk_polyvec[0] mod (X^256+1)` 만 곱.

run::
    python3 scripts/s3_v_capture_main.py -n 200 \\
        --out traces/s3_v_skX_a4_n200.npz
"""

from __future__ import annotations

import argparse
import hashlib
import sys
import time
from datetime import datetime, timezone
from pathlib import Path

_REPO = Path(__file__).resolve().parents[2]
if str(_REPO) not in sys.path:
    sys.path.insert(0, str(_REPO))

import numpy as np  # noqa: E402

from host.smaug.chosen_ct import reset_target, setup_session  # noqa: E402
from host.smaug import chosen as _chosen  # noqa: E402
from host.smaug import codec as _codec  # noqa: E402
from host.smaug import params as _params  # noqa: E402
from scripts.smaug.s1_t_roundtrip import dump_sk_pke  # noqa: E402


def _file_sha256(p: Path) -> str:
    h = hashlib.sha256()
    with p.open("rb") as fh:
        for chunk in iter(lambda: fh.read(1 << 16), b""):
            h.update(chunk)
    return h.hexdigest()


def parse_args() -> argparse.Namespace:
    pa = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    pa.add_argument("-n", "--num-traces", type=int, default=200)
    pa.add_argument("-s", "--samples", type=int, default=24400)
    pa.add_argument("-g", "--gain-db", type=float, default=25.0)
    pa.add_argument("--adc-offset", type=int, default=0)
    pa.add_argument("--component", type=int, default=0, choices=(0, 1))
    pa.add_argument("--coef", type=int, default=0)
    pa.add_argument("--alpha", type=int, default=4,
                    help="c1[component, coef] — α=4 권장 (S2 와 동일, "
                         "round-trip predict 통과 = HW(µ')=0 trivial).")
    pa.add_argument(
        "--firmware-hex",
        type=Path,
        default=_REPO / "firmware" / "simpleserial-smaug" /
        "simpleserial-smaug-CW308_STM32F4.hex",
    )
    pa.add_argument("--baud", type=int, default=38400)
    pa.add_argument("--serial", default=None)
    pa.add_argument("--out", type=Path, default=None)
    return pa.parse_args()


def _pack_mu_bits(mu_bits: np.ndarray) -> bytes:
    out = bytearray(32)
    for bit_i in range(256):
        if mu_bits[bit_i]:
            out[bit_i // 8] |= 1 << (bit_i % 8)
    return bytes(out)


def main() -> int:
    args = parse_args()
    p = _params.get("smaug1")
    p.assert_consistent()
    if not (0 <= args.alpha < p.p):
        raise SystemExit(f"--alpha {args.alpha} ∉ [0, {p.p})")
    out_path = args.out
    if out_path is None:
        tag = f"c{args.component}_j{args.coef}_a{args.alpha}_n{args.num_traces}"
        out_path = _REPO / "traces" / f"s3_v_{tag}_TEMP.npz"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    fw_sha = _file_sha256(args.firmware_hex) if args.firmware_hex.exists() else None
    print(f"[INFO] firmware_hex={args.firmware_hex.name} sha256={fw_sha}")

    ct = _chosen.build_monomial_c1(
        p, component=args.component, coef_idx=args.coef, alpha=args.alpha
    )
    ct_bytes = ct.to_bytes()
    if len(ct_bytes) != p.ciphertext_bytes:
        raise SystemExit(
            f"ct len {len(ct_bytes)} != CIPHERTEXT_BYTES {p.ciphertext_bytes}"
        )
    print(
        f"[INFO] chosen-CT c{args.component}·X^{args.coef}·{args.alpha}, "
        f"len={len(ct_bytes)}B"
    )

    import chipwhisperer as cw  # noqa: E402
    from host.cw_serial import pick_serial  # noqa: E402

    sn = pick_serial(args.serial)
    print(f"[INFO] CW1173 sn={sn}")
    scope = cw.scope(sn=sn)
    scope.default_setup()
    scope.adc.samples = args.samples
    scope.adc.offset = args.adc_offset
    scope.adc.basic_mode = "rising_edge"
    scope.gain.db = args.gain_db
    scope.clock.adc_src = "clkgen_x4"
    scope.trigger.triggers = "tio4"
    reset_target(scope)
    target = cw.target(scope, cw.targets.SimpleSerial)
    target.baud = args.baud
    target.flush()
    time.sleep(0.3)

    started = time.time()
    try:
        # 1) F (fresh keypair, sk persistent)
        target.simpleserial_write("F", b"")
        pk_fp16 = bytes(target.simpleserial_read("r", 16, timeout=5000))
        print(f"[OK] F pk_fp16={pk_fp16.hex()}")

        # 2) X — sk PKE 32B chunks
        sk_pke = dump_sk_pke(target, p.pke_secret_key_bytes // 32)
        print(f"[OK] X sk_pke[0:8]={sk_pke[:8].hex()}…")
        sk_unpacked_full = _codec.unpack_sx(sk_pke).astype(np.int64)
        sk_polyvec = sk_unpacked_full.reshape(p.module_rank, p.n)

        # 3) inject ct into the resident key session. Do not call 'F' again:
        # sk_pke above must label the exact key used by this 'V' capture.
        bundle = setup_session(
            target,
            ct_bytes,
            label="s3_v_chosen",
            fresh_key=False,
            pk_fp16=pk_fp16,
        )
        if bundle.ct_fp16_host != bundle.ct_fp16_board:
            raise RuntimeError(
                f"ct integrity fail: host={bundle.ct_fp16_host.hex()} "
                f"board={bundle.ct_fp16_board.hex()}"
            )
        print(f"[OK] inject + 'L' verify ct_fp16={bundle.ct_fp16_board.hex()}")

        # 4) round-trip — V µ' vs predict (only meaningful for HW=0 designs like α=4)
        target.simpleserial_write("V", b"")
        first_mu_resp = bytes(target.simpleserial_read("r", 32, timeout=5000))
        if len(first_mu_resp) != 32:
            raise RuntimeError("V first ack fail")
        mu_pred = _chosen.predict_mu_prime(p, ct.c1, sk_polyvec, ct.c2)
        mu_pred_packed = _pack_mu_bits(mu_pred)
        if mu_pred_packed != first_mu_resp:
            n_diff_bits = 0
            for byte_i in range(32):
                n_diff_bits += bin(mu_pred_packed[byte_i] ^ first_mu_resp[byte_i]).count("1")
            print(
                f"[WARN] µ′ round-trip {n_diff_bits}/256 bits differ. "
                f"host_pred={mu_pred_packed.hex()}  board={first_mu_resp.hex()}\n"
                f"       (predict_mu_prime 의 unsigned-c1 buggy 가능; α=4 default 면 "
                f"통과해야 함. 진행 — V==Z 자체는 별도 검증 됨.)"
            )
            if args.alpha == 4:
                # for default α=4, must pass — else firmware is wrong
                raise RuntimeError(
                    "α=4 round-trip 실패 — firmware/V command 자체 문제."
                )
        else:
            print("[OK] µ′ round-trip 256/256 bits 일치")

        # 5) V × N capture
        traces = np.empty((args.num_traces, args.samples), dtype=np.float32)
        acks = np.zeros((args.num_traces, 32), dtype=np.uint8)
        timeouts = 0
        n_ok = 0
        for i in range(args.num_traces):
            scope.arm()
            target.simpleserial_write("V", b"")
            if scope.capture():
                timeouts += 1
                print(f"[WARN] scope timeout @ {i}")
                continue
            ack = target.simpleserial_read("r", 32, timeout=5000)
            if ack is None or len(ack) != 32:
                timeouts += 1
                print(f"[WARN] short ack @ {i}")
                continue
            traces[i] = scope.get_last_trace().astype(np.float32)
            acks[i] = bytearray(ack)
            n_ok += 1
            if (i + 1) % 100 == 0:
                el = time.time() - started
                print(f"[PROG] {i+1}/{args.num_traces}  ({el:.1f}s, {(i+1)/el:.2f} tr/s)")

        unique_acks = np.unique(acks[:n_ok], axis=0)
        if unique_acks.shape[0] != 1:
            raise RuntimeError(
                f"ack 다양성 {unique_acks.shape[0]} — sk 변경 또는 µ′ 비결정성 의심"
            )
        if bytes(acks[0]) != first_mu_resp:
            raise RuntimeError("first capture ack != initial round-trip ack")
        print(f"[OK] {n_ok} traces ack 일관성")

        try:
            tc = scope.adc.trig_count
        except Exception:
            tc = -1
        print(f"[INFO] scope.adc.trig_count = {tc}  (expected ≈ 8.4M for V)")

        if "TEMP" in str(out_path):
            tag2 = pk_fp16.hex()[:8]
            out_path = out_path.with_name(out_path.name.replace("TEMP", tag2))

        meta = {
            "captured_at": datetime.now(timezone.utc).isoformat(),
            "scope_sn": sn,
            "samples": args.samples,
            "gain_db": args.gain_db,
            "adc_offset": args.adc_offset,
            "target": "smaug1",
            "cmd": "V",
            "level": "smaug1",
            "log_p": p.log_p,
            "module_rank": p.module_rank,
            "lwe_n": p.n,
            "ss_ver": "SS_VER_1_1",
            "baud": args.baud,
            "n_attempted": args.num_traces,
            "n_ok": n_ok,
            "n_timeouts": timeouts,
            "firmware_hex": str(args.firmware_hex),
            "firmware_hex_sha256": fw_sha,
            "pk_fp16": pk_fp16.hex(),
            "sk_pke_hex": sk_pke.hex(),
            "ct_bytes_sha256": hashlib.sha256(ct_bytes).hexdigest(),
            "ct_fp16_board": bundle.ct_fp16_board.hex(),
            "ct_fp16_host": bundle.ct_fp16_host.hex(),
            "ct_label": f"c{args.component}_X{args.coef}_a{args.alpha}",
            "alpha": args.alpha,
            "component": args.component,
            "coef_idx": args.coef,
            "first_mu_resp_hex": first_mu_resp.hex(),
            "trig_count": int(tc),
            "label": f"s3_v_{out_path.stem}",
        }
        np.savez_compressed(
            out_path,
            traces=traces[:n_ok],
            acks=acks[:n_ok],
            meta=np.array(meta, dtype=object),
        )
        el = time.time() - started
        print(f"[OK] saved {n_ok}/{args.num_traces} -> {out_path}  ({el:.1f}s)")

    finally:
        try: target.dis()
        except Exception: pass
        try: scope.dis()
        except Exception: pass
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
