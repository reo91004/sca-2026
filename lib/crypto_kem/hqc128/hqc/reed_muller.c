...
#ifdef RM_DEBUG
    printf("\n========================================\n");
    printf("[RM_DECODE] reed_muller_decode() started\n");
    printf("[RM_DECODE] Processing %d bytes\n", VEC_N1_SIZE_BYTES);
    printf("========================================\n");
#endif

    for (size_t i = 0; i < VEC_N1_SIZE_BYTES; i++) {
#ifdef RM_DEBUG
        if (i < 3) {  // 처음 3바이트만 상세 출력
            printf("\n[RM_DECODE] Decoding byte %zu:\n", i);
            rm_codeword_t *block = &codeArray[i * MULTIPLICITY];
            for (int j = 0; j < MULTIPLICITY; j++) {
                printf("  Block[%d]: %08x %08x %08x %08x\n", j,
                       block[j].u32[0], block[j].u32[1],
                       block[j].u32[2], block[j].u32[3]);
            }
        }
#endif
...
#ifdef RM_DEBUG
        if (i < 3) {
            printf("  After expand_and_sum:\n");
            printf("    expanded[0-3]: %d %d %d %d\n",
                   expanded[0], expanded[1], expanded[2], expanded[3]);
            printf("    expanded[124-127]: %d %d %d %d\n",
                   expanded[124], expanded[125], expanded[126], expanded[127]);
        }
#endif
...
#ifdef RM_DEBUG
        if (i < 3) {
            printf("  After fixing transform[0]: %d\n", transform[0]);
        }
#endif
...
#ifdef RM_DEBUG
    printf("\n[RM_DECODE] First 10 decoded bytes: ");
    for (int i = 0; i < 10 && i < VEC_N1_SIZE_BYTES; i++) {
        printf("%02x ", message_array[i]);
    }
    printf("\n========================================\n\n");
#endif
}
